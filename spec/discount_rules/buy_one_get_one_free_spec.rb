...
RSpec.describe DiscountRules::BuyOneGetOneFree do
  subject(:bogof_rule) { described_class.new(product_code: 'GR1') }
  ...
  describe '#total_amount' do
    context 'with an odd number of items' do
      ...
      it 'charges full price for 1 item' do
        expect(bogof_rule.total_amount(product: green_tea, quantity: 1)).to eq(311)
      end

      # 3 items = pay for 2 = 2 * 311 = 622
      it 'charges for 2 when buying 3' do
        expect(bogof_rule.total_amount(product: green_tea, quantity: 3)).to eq(622)
      end

      # 5 items = pay for 3 = 3 * 311 = 933
      it 'charges for 3 when buying 5' do
        expect(bogof_rule.total_amount(product: green_tea, quantity: 5)).to eq(933)
      end
    end
    ...
    context 'with an even number of items' do
      ...
      it 'charges for 1 when buying 2' do
        expect(bogof_rule.total_amount(product: green_tea, quantity: 2)).to eq(311)
      end

      # 4 items = pay for 2 = 2 * 311 = 622
      it 'charges for 2 when buying 4' do
        expect(bogof_rule.total_amount(product: green_tea, quantity: 4)).to eq(622)
      end
    end
    ...
    context 'with zero items' do
      ...
      it 'returns zero' do
        expect(bogof_rule.total_amount(product: green_tea, quantity: 0)).to eq(0)
      end
    end
  end
end